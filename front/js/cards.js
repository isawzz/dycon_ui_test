...
function set_card_constants(w, h, ranks, suits, deckletters, numjokers = 0, ovdeck = .25, ovw = '20%', ovh = '20%') {
	...
	CSZ = h;
	CHEIGHT = h;
	CWIDTH = isdef(w) ? w : CSZ * .7;
	CGAP = CSZ * .05;
	OVD = ovdeck;
	OVW = isString(ovw) ? CWIDTH * firstNumber(ovw) / 100 : ovw;
	OVH = isString(ovh) ? CHEIGHT * firstNumber(ovh) / 100 : ovh;
	RANKS = ranks; //'A23456789TJQK';
	SUITS = suits; //'SHDC';
	DECKS = deckletters; //colors of backside rbgyop (red,blue,green,yellow,orange,purple)
	NUMDECKS = deckletters.length; //2;
	NUMJOKERS = numjokers;
}
	...
	for (const r of RANKS) {
		for (const s of SUITS) {
			for (const c of DECKS) {
				...
				di[k] = { key: k, val: r == 'A' ? 1 : 'TJQK'.includes(r) ? 10 : Number(r), rank: r, suit: s, color: colors[c], c52key: 'card_' + r + s, w: sz * .7, h: sz, sz: sz, ov: OVW, friendly:`${isNumber(r)?r:ranknames[r]} of ${suitnames[s]}`, short:`${r}${s}` };
			}
		}
	}
	Aristocards = di;
...
function ari_get_card(ckey, h, w, ov) {
	...
	[w, h] = [isdef(w) ? w : isdef(info.w) ? info.w : CWIDTH, isdef(h) ? h : isdef(info.h) ? info.h : CSZ];
	mSize(el, w, h);
	let res = {};
	copyKeys(info, res);
...
function get_card_div(R1 = '1', SB = 'B') {
	...
	svgCode = '<div>' + svgCode + '</div>';
	let el = createElementFromHTML(svgCode);
	[w, h] = [isdef(w) ? w : CWIDTH, isdef(h) ? h : CSZ];
	mSize(el, w, h);
	return el;
}
...
function inno_card(dParent, keyOrName) {
	...
	let card = cBlank(dParent, { fg: 'black', bg: INNO.color[cardInfo.color], w: CSZ, h: CSZ * .65, margin: 10 });
	let [dCard, sz, szTitle, margin] = [iDiv(card), CSZ / 5, cardInfo.exp[0] == 'A' ? CSZ / 12 : CSZ / 8, 4];
